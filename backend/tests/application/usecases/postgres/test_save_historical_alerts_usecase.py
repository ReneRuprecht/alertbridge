...
@pytest.mark.asyncio
async def test_save_historical_alerts_returns_repo_count_and_passes_input():
    mock_repo = AsyncMock(spec=AlertHistoricalRepositoryPort)
    mock_repo.save_historical_alerts.return_value = 1

    usecase = SaveHistoricalAlertsUseCase(db_repo=mock_repo)
    ...
    mock_repo.save_historical_alerts.assert_awaited_once_with(
        alerts=alerts_to_save
    )


@pytest.mark.asyncio
async def test_save_historical_alerts_with_empty_list():
    mock_repo = AsyncMock(spec=AlertHistoricalRepositoryPort)
    mock_repo.save_historical_alerts.return_value = 0
